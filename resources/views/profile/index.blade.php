        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <div class="flex items-center space-x-6 pb-6 border-b dark:border-gray-700">
                    <div class="shrink-0">
                        <img class="h-32 w-32 rounded-full object-cover border-4 border-gray-100 dark:border-gray-700 shadow-lg" 
                             src="https://ui-avatars.com/api/?name={{ urlencode(session('user')->name) }}&background=0D8ABC&color=fff" 
                             alt="Profile avatar">
                    </div>
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ session('user')->name }}</h1>
                        <p class="text-gray-500 dark:text-gray-400">Member since {{ session('user')->created_at->format('F Y') }}</p>
                    </div>
                </div>
                ...
                <div class="mt-8 grid gap-6 md:grid-cols-2">
                    <!-- Email Section -->
                    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow duration-300">
                        <div class="p-6">
                            ...
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-600 dark:text-gray-400">Last updated {{ session('user')->updated_at->diffForHumans() }}</p>
                                    </div>
                                </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 border border-gray-100 dark:border-gray-600">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900 dark:text-white">Active</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Account Type</p>
                                <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ ucfirst(session('user')->role ?? 'User') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
